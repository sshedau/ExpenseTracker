...
export default function ForgotPassword() {
  ...
  const sendResetEmail = async () => {
    ...
    try {
      ...
      // Backend generated reset link
      const resetLink = data.resetLink;

      if (!resetLink) {
        throw new Error(
          "Reset link was not generated by the server."
        );
      }
        ...
        <div className="w-full max-w-md">
          ...
          <div className="relative rounded-2xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700/60 shadow-xl shadow-slate-200/60 dark:shadow-slate-900/60 p-8">
            ...
            <button
              ...
              disabled={loading}
              className={`mt-6 w-full py-3 rounded-xl text-white text-sm font-bold tracking-wide transition-all duration-150 shadow-md ${
                loading
                  ? "bg-indigo-400 cursor-not-allowed"
                  : "bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 shadow-indigo-500/30 hover:shadow-lg hover:shadow-indigo-500/40"
              }`}
            >
              {loading
                ? "Sending..."
